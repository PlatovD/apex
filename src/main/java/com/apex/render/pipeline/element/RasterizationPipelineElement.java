...
public class RasterizationPipelineElement implements PipelineElement {
    ...
    public void apply(RenderObject ro) {
        ...
        for (Polygon polygon : ro.getModel().polygons) {
            ...
            if (!isOnScreen(vertex0Attribute, vertex1Attribute, vertex2Attribute, Constants.SCENE_WIDTH,
    ...
    private boolean isOnScreen(VertexAttribute v0, VertexAttribute v1, VertexAttribute v2, int screenWidth,
            int screenHeight) {
        // Simple but robust culling: if ANY vertex is behind the eye (z < -1 in
        // normalized coords),
        // we skip the triangle. Correct clipping would be better, but this prevents
        // crashes.
        ...
        boolean fairer = v0.z > 1 && v1.z > 1 && v2.z > 1; // Only cull if ALL are too far

        return !(lefter || righter || toper || downer || fairer);
    }
