...
import com.apex.tool.modification.ModelTools;
...
public abstract class AbstractController implements Controller {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        collapseSection(lightsVBox, lightsCollapseBtn);
    ...
    @AutoInject
    protected com.apex.storage.LightStorage lightStorage;

    @FXML
    protected VBox lightsVBox;
    @FXML
    protected Button lightsCollapseBtn;
...
// refreshLightList();
    ...
    @FXML
    public void handleLightsCollapseToggle(MouseEvent event) {
        // Используем ваш существующий метод
        toggleCollapse(lightsVBox, lightsCollapseBtn);
    }

    @FXML
    protected void handleLightAdd() {
        executeSafe("Adding Light", () -> {
            // 1. Опрашиваем координаты через диалог
            javafx.scene.control.TextInputDialog dialog = new javafx.scene.control.TextInputDialog("0.0 0.0 0.0");
            dialog.setTitle("New Light Source");
            dialog.setHeaderText("Enter light position (X Y Z):");
            dialog.setContentText("Coordinates:");

            dialog.showAndWait().ifPresent(input -> {
                String[] parts = input.trim().split("\\s+");
                if (parts.length != 3) {
                    throw new IllegalArgumentException("Please enter exactly three numbers (e.g. 10.5 20 0)");
                }
                ...
                String name = "Light_" + (lightStorage.getLightsNames().size() + 1);
                lightStorage.addLight(name, new Light(new Vector3f(x, y, z), 0xFFFFFFFF));

                // 3. Обновляем UI и рендер
                refreshLightList();
                refreshRender();
            });
        });
    ...
    protected void refreshLightList() {
        Platform.runLater(() -> {
            ...
            for (String name : lightStorage.getLightsNames()) {
                ...
                ColorPicker cp = new ColorPicker(ColorUtil.fromARGB(light.getColor()));
                cp.setPrefWidth(42);
                cp.setOnAction(e -> {
                    lightStorage.updateLightColor(name, ColorUtil.toARGB(cp.getValue()));
                    refreshRender();
                });

                // Удаление (кроме дефолтного)
                Button delBtn = new Button("×");
                delBtn.getStyleClass().add("model-btn");
                delBtn.setDisable(name.equals(com.apex.core.Constants.DEFAULT_LIGHT_NAME));
                delBtn.setOnAction(e -> {
                    lightStorage.deleteLight(name);
                    refreshLightList();
                    refreshRender();
                });
